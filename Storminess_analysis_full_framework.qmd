...
This diagnostic is the single most important prerequisite for the entire statistical framework. Standard statistical tests, including Ordinary Least 
Squares (OLS) regression, operate on the strict assumption that the data points (or model residuals) are independent. Climate time series violate this  assumption due to strong persistence, or "memory," arising from the inertia of the climate system.

### 1.4.1 Autocorrelation and Partial Autocorrelation Functions

To quantify the degree of persistence in the area-weighted Pan-Arctic storminess metrics, both the Autocorrelation Function (ACF) and Partial Autocorrelation Function (PACF) were computed for lags up to 36 months. The ACF measures the correlation between observations separated by $k$ time steps:
...
| Metric          | Lag-1 ACF ($\rho_1$) | Lag-1 PACF ($\phi_1$) | $n_{eff}$ | DoF Reduction |
|-----------------|----------------------|-----------------------|-----------|---------------|
| eke_mean        | 0.648                | 0.648                 | 592       | 27.4%         |
| eke_p90         | 0.649                | 0.649                 | 599       | 26.6%         |
| eke_p95         | 0.658                | 0.658                 | 595       | 27.1%         |
| eke_p99         | 0.664                | 0.664                 | 600       | 26.5%         |
| exceed_p95_freq | 0.583                | 0.583                 | 571       | 30.1%         |

: Autocorrelation diagnostics for Pan-Arctic storminess metrics {#tbl-acf}

All lag-1 values substantially exceed the threshold of $\rho_1 > 0.1$ that indicates violation of the independence assumption for monthly atmospheric data (Santer et al. 2000). The PACF exhibits a significant spike at lag 1 followed by rapid decay toward the confidence bounds at subsequent lags, confirming that an AR(1) error structure is appropriate for GLS regression analyses. Secondary significant spikes at lags 12 and 24 reflect residual seasonality, consistent with the strong seasonal cycle identified in the STL decomposition.
...
cat("\n--- Generating Plot 2: ACF and PACF Plots ---\n")

# Create multi-panel ACF/PACF plot
# Layout: 5 metrics × 2 columns (ACF, PACF)
png(file.path(fig_dir, "Fig02_ACF_PACF_AllMetrics.png"),
...
for (metric in METRICS) {
  if (metric %in% names(ts_objects)) {
    ...
    # Get PACF lag-1 value
    pacf_result <- pacf(ts_objects[[metric]], lag.max = 36, plot = FALSE)
    pacf1 <- pacf_result$acf[1]
    ...
    # PACF plot
    pacf(ts_objects[[metric]], lag.max = 36,
         main = paste0("PACF: ", metric),
         xlab = "Lag (months)",
         ylab = "Partial Autocorrelation",
         col = metric_colors[metric],
         lwd = 2)
    mtext(bquote(phi[1] == .(sprintf("%.3f", pacf1))), 
          side = 3, line = -1.5, adj = 0.95, cex = 0.8)
  }
}

mtext("Autocorrelation and Partial Autocorrelation Functions: Pan-Arctic Storminess Metrics (1958–2025)",
      outer = TRUE, line = 1, cex = 1.2, font = 2)

dev.off()
cat("  ✓ Saved: Fig02_ACF_PACF_AllMetrics.png\n")
