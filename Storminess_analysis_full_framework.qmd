...
-   PHASE 1: Data Preparation & Validation
    ...
    -   1.6 Energy budget characterisation:

        -   1.6.1 Partitioning climatology (fEKE, MKE/EKE ratio)

        -   1.6.2 Seasonal cycle of energy components

        -   1.6.3 EKE definition comparison (budget vs synoptic, r = 0.885)

-   PHASE 2: Trend analysis

    -   2.1 Pan-Arctic trend analysis (all storminess metrics + energy components, simultaneously)

    -   2.2 Seasonal decomposition (all metrics by DJF/MAM/JJA/SON)

    -   2.3 Autumn intensification hypothesis test

    -   2.4 Spatial trend patterns + field significance

    -   2.5 Period comparison (pre- vs. post-regime shift, if warranted by 2.1–2.3)

-   PHASE 3: Regional Sub-Analysis (apply full Phase 3-4 workflow to each region)

    -   3.1 Fram Strait sector

    -   3.2 West Svalbard sector (10°E–16°E, 76°N–81°N)

    -   3.3 East Svalbard sector (16°E–28°E, 76°N–81°N)

-   PHASE 5: Synthesis and hypothesis testing

    -   4.1 Summary of all trend results

    -   4.2 Autumn feedback loop hypothesis evaluation

    -   4.3 Comparison with sea-ice loss timelines
...
# --------------------------------------------------------------------------
# 1.5.1  Load and Spatially Aggregate Energy Components
# --------------------------------------------------------------------------
...
cat("  ✓ Temporal alignment verified (both have 816 months)\n")
```

```{r}
# --------------------------------------------------------------------------
# 1.5.2  Budget Closure Verification
# --------------------------------------------------------------------------
...
```{r}
# ============================================================================
# Section 1.5.3: SAVE PREPARED DATA
# ============================================================================
cat("\n", rep("-", 40), "\n")
...
mtext("Autocorrelation Functions: Energy Components (Deseasonalized, 1958–2025)",
      outer = TRUE, line = 1, cex = 1.2, font = 2)
dev.off()
cat("  ✓ Saved: Fig11_ACF_PACF_Energy_Anomalies.png\n")
...
cat("\nGenerated Figures:\n")
cat("  Fig11: ACF/PACF for energy components (deseasonalised)\n")


# ============================================================================
# UPDATE: SAVE SECTION
# ============================================================================
# The existing save block should be updated to include the new energy 
# budget objects. Add these to the save() call:
#
#   energy_annual,
#   acf_results_energy,
#   acf_comparison_energy,
#   relative_error,        # budget closure error (%)
#
# storm_df now includes the energy columns: tke, mke, eke_budget,
# eke_fraction, mke_eke_ratio, and the _anom columns.
#
# Phase 2 (Energy Budget Characterisation) loads this .RData file
# and generates its own figures in a separate script.
# ============================================================================
...
#   Each phase has a single, well-defined purpose.
#
# INPUT:
...
cat("  ✓ Saved: Fig2.1_Energy_Budget_Partitioning.png\n\n")
```
...
energy_seasonal <- storm_df %>%
  ...
  summarise(
    tke_mean        = mean(tke),
    mke_mean        = mean(mke),
    eke_mean_budget = mean(eke_budget),
    .groups         = "drop"
  )

# Report winter/summer contrast
winter_eke <- mean(energy_seasonal$eke_mean_budget[energy_seasonal$month %in% c(12, 1, 2)])
summer_eke <- mean(energy_seasonal$eke_mean_budget[energy_seasonal$month %in% c(6, 7, 8)])
...
seasonal_long <- energy_seasonal %>%
  select(month, tke_mean, mke_mean, eke_mean_budget) %>%
  pivot_longer(-month, names_to = "component", values_to = "value") %>%
  mutate(
    component = factor(component,
                       levels = c("tke_mean", "mke_mean", "eke_mean_budget"),
...
cat("  ✓ Saved: Fig2.2_Energy_Seasonal_Cycle.png\n\n")
```
...
# Compute correlation
eke_cor <- cor(storm_df$eke_budget, storm_df$eke_mean, use = "complete.obs")
cat(sprintf("  Pearson r (monthly): %.3f\n", eke_cor))

# Compute synoptic fraction
storm_df <- storm_df %>%
  mutate(f_synoptic = eke_mean / eke_budget)
...
comparison_annual <- energy_annual %>%
  select(year, eke_budget, eke_mean) %>%
  pivot_longer(-year, names_to = "type", values_to = "value") %>%
  mutate(
    type = factor(type,
                  levels = c("eke_budget", "eke_mean"),
...
cat("  ✓ Saved: Fig2.3_EKE_Comparison.png\n")
```
