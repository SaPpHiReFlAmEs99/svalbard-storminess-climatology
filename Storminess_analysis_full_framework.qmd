...
lon_labels <- data.frame(lon = seq(0, 300, by = 60), lat = 62) %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326) %>%
  st_transform(crs = arctic_crs)
...
make_panel <- function(stars_obj, fill_var, fill_scale, panel_title,
  ...
  if (!is.null(sig_contour)) {
    # st_warp interpolates 0/1 to continuous values; threshold at 0.5
    # and map directly to alpha via scale_alpha_identity()
    sig_contour[[1]] <- ifelse(
      is.na(sig_contour[[1]]) | sig_contour[[1]] < 0.5, 0, 0.35
    )
    p <- p +
      geom_stars(data = sig_contour, aes(alpha = sig_mask),
                 fill = "black", color = NA, na.rm = TRUE) +
...
var_plot_config <- list(
  ...
  exceed_p95_freq = list(
    ...
    display_name = "P95 Exceedance Freq."
  ),
  tke = list(
...
for (vname in names(spatial_results)) {
  ...
  p_ann <- make_panel(
    ...
    panel_title = bquote(bold("Annual Sen slope") ~ .(cfg$display_name) ~
                           "(1958\u20132025)"),
