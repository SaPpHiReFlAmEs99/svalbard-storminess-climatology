...
panel_theme <- theme_void(base_size = 12) +
  theme(
...
p6a_p90 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('"(b) "~', VAR_LABELS_SHORT[["eke_p90"]]))[[1]]) +
  panel_theme
...
p6a_p95 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('"(c) "~', VAR_LABELS_SHORT[["eke_p95"]]))[[1]]) +
  panel_theme
...
p6a_p99 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('"(d) "~', VAR_LABELS_SHORT[["eke_p99"]]))[[1]]) +
  panel_theme
...
# ── Pre-Whitened Mann-Kendall (iterative Zhang method) ───────────────
  ...
  #   5. Remove β̂ from the original series to obtain detrended residuals.
...
#   - phase3-1 / phase3-2 chunks (sen_slope() in environment)z
#   - Phase 1 prepared data: PHASE1_DATA (for gridded arrays)
#   - NetCDF files: METRICS_FILE (direct read for gridded data)
#
# PURPOSE:
#   Apply the Pre-Whitened Mann-Kendall (PWMK) test and Sen's slope
#   estimator at every ERA5 grid point independently, then apply the
#   Benjamini-Hochberg (BH) False Discovery Rate correction across
#   all simultaneous tests to identify field-significant trends.
#
...
#
#   Step 3 — Benjamini-Hochberg False Discovery Rate correction
#     [p.adjust(method = "BH")]
#     The multiple testing problem: at alpha = 0.05, approximately
#     0.05 x K ~ 8,640 grid points would appear significant by chance
#     alone under a true global null hypothesis. The BH procedure
#     controls the EXPECTED PROPORTION of false discoveries among all
#     rejected tests (Benjamini & Hochberg 1995), rather than the
#     probability of any false discovery (Bonferroni). This distinction
#     is crucial: BH is substantially more powerful than Bonferroni
#     while remaining valid under positive dependence between spatially
#     correlated grid points (Benjamini & Yekutieli 2001), as confirmed
#     for climate fields by Wilks (2006) and Ventura et al. (2004).
#
#     The BH threshold is applied at alpha_FDR = 0.10, following the
#     recommendation of Wilks (2016) for spatial climate analyses. Under
#     spatial correlation, the BH procedure becomes conservative relative
#     to its nominal level; using alpha_FDR = 2 x alpha_local recovers
#     approximately the intended control level for typical climate fields.
#
...
#   on figure aesthetics without re-running ~1M+ PWMK tests.
#   To force recomputation, delete the .RData file or set
#   FORCE_RECOMPUTE <- TRUE before running this chunk.
#
# OUTPUT:
#   spatial_results       — named list by variable, each containing
#                           annual and seasonal trend grids
#   fdr_summary           — data frame summarising FDR results
#   Fig3.5_*              — Annual + seasonal trend maps per variable
...
# ── Configuration ────────────────────────────────────────────────────────────
ALPHA_FDR <- 0.10   # Wilks (2016, BAMS): alpha_FDR = 2 * alpha_local

# Variables to analyse:
#   - 5 storminess metrics (all showed significance in MAM or are near-
#     significant; spatial analysis may reveal localised signals masked
#     by Pan-Arctic aggregation)
#   - TKE (the only energy component with significant Pan-Arctic trends)
SPATIAL_VARS <- c(
...
if (file.exists(RESULTS_FILE) && !FORCE_RECOMPUTE) {
  ...
  # Verify that all requested variables are present in cached results.
  # If new variables were added to SPATIAL_VARS since the last run,
...
if (!file.exists(RESULTS_FILE) || FORCE_RECOMPUTE) {
  ...
  # BH-FDR on a [lat x lon] p-value matrix
  apply_bh_fdr <- function(plocal_mat, alpha_fdr = ALPHA_FDR) {
    p_vec    <- as.vector(plocal_mat)
    padj_vec <- rep(NA_real_, length(p_vec))
    valid    <- !is.na(p_vec)
    padj_vec[valid] <- p.adjust(p_vec[valid], method = "BH")
    padj_mat <- matrix(padj_vec, nrow(plocal_mat), ncol(plocal_mat))
    sig_mat  <- padj_mat <= alpha_fdr
    list(
      p_adjusted  = padj_mat,
      significant = sig_mat,
      n_sig       = sum(sig_mat, na.rm = TRUE),
      n_total     = sum(!is.na(plocal_mat))
    )
  }
  ...
  for (vi in seq_along(SPATIAL_VARS)) {
    ...
    fdr_ann  <- apply_bh_fdr(pwmk_ann$p_local)

    progress_msg(sprintf("    [%s] Annual FDR: %d / %d sig (%.1f%%) at alpha_FDR = %.2f",
                          vname, fdr_ann$n_sig, fdr_ann$n_total,
                          100 * fdr_ann$n_sig / fdr_ann$n_total, ALPHA_FDR))
    ...
    spatial_results[[vname]] <- list(
      annual = list(
        ...
        tau         = pwmk_ann$tau,
        p_adjusted  = fdr_ann$p_adjusted,
        significant = fdr_ann$significant,
        years       = years_vec
      ),
      seasonal = list()
    )

    rm(annual_arr, pwmk_ann, fdr_ann); gc(verbose = FALSE)
    ...
    for (si in seq_along(SEASONS)) {
      ...
      fdr_s    <- apply_bh_fdr(pwmk_s$p_local)

      progress_msg(sprintf("    [%s] %s FDR: %d / %d sig (%.1f%%)",
                            vname, seas, fdr_s$n_sig, fdr_s$n_total,
                            100 * fdr_s$n_sig / fdr_s$n_total))

      spatial_results[[vname]]$seasonal[[seas]] <- list(
        slope        = pwmk_s$slope,
        p_local      = pwmk_s$p_local,
        tau          = pwmk_s$tau,
        p_adjusted   = fdr_s$p_adjusted,
        significant  = fdr_s$significant,
        season_years = seas_agg$years
      )

      rm(seas_agg, pwmk_s, fdr_s); gc(verbose = FALSE)
    }

    # ── Intermediate save after each variable ──────────────────────────────
    # Guards against crashes: partial results are recoverable.
    save(spatial_results, ALPHA_FDR, SPATIAL_VARS, lat_arc, lon_vec,
         file = RESULTS_FILE)
  ...
  # ========================================================================
  # A4: BUILD SUMMARY TABLE AND FINAL SAVE
  # ========================================================================
  progress_msg("A4: Building summary table and saving final results...")
  ...
  for (vname in names(spatial_results)) {
    ...
    # Annual
    fdr_a <- apply_bh_fdr(res$annual$p_local)
    summary_rows[[length(summary_rows) + 1L]] <- data.frame(
      variable = vname, period = "Annual",
      n_sig = fdr_a$n_sig, n_total = fdr_a$n_total,
      pct_sig = round(100 * fdr_a$n_sig / fdr_a$n_total, 2),
      stringsAsFactors = FALSE
    )

    # Seasonal
    for (seas in SEASONS) {
      fdr_s <- apply_bh_fdr(res$seasonal[[seas]]$p_local)
      summary_rows[[length(summary_rows) + 1L]] <- data.frame(
        variable = vname, period = seas,
        n_sig = fdr_s$n_sig, n_total = fdr_s$n_total,
        pct_sig = round(100 * fdr_s$n_sig / fdr_s$n_total, 2),
        stringsAsFactors = FALSE
      )
    }
  }
  fdr_summary <- bind_rows(summary_rows)

  # Final save (includes fdr_summary)
  save(spatial_results, ALPHA_FDR, SPATIAL_VARS, lat_arc, lon_vec,
       fdr_summary,
       file = RESULTS_FILE)
...
# Derive grid dimensions from lat_arc / lon_vec, which are always available
# (either from the computation block above or restored from the .RData cache).
# n_lat / n_lon are NOT saved in the cache, so this must run on every execution.
n_lat <- length(lat_arc)
n_lon <- length(lon_vec)
...
# Print summary table
cat("FDR SIGNIFICANCE SUMMARY (alpha_FDR =", ALPHA_FDR, ")\n")
cat(rep("-", 60), "\n")
...
make_stars_polar <- function(mat, varname = "value") {
  ...
  mat_wrap <- t(mat_wrap)    # [lat, lon] -> [lon, lat] to match stars x/y convention
...
make_panel <- function(stars_obj, fill_var, fill_scale, panel_title,
  ...
  p <- ggplot() +
    ...
    geom_sf(data = inverse_mask, fill = "white", color = NA) +
    geom_sf_text(data = lat_labels, aes(label = label),
                 size = 2.3, color = "black", hjust = 1.2) +
    geom_sf_text(data = lon_labels, aes(label = label),
                 size = 2.3, color = "black")

  if (!is.null(sig_contour)) {
    # st_warp interpolates the binary 0/1 mask to continuous values.
    # Threshold at 0.5 to recover significance: keep 1L, set everything else NA.
    # Then convert to sf polygons with st_as_sf(na.rm = TRUE, merge = TRUE):
    # this operates at the geometry level, so only the significant cells
    # produce polygons — NA cells produce nothing. geom_sf then overlays
    # those polygons with a constant black fill, which has no conflict with
    # the slope fill scale already in the plot (unlike a second geom_stars
    # layer, which would render the full rectangular raster extent).
    ...
    p <- p +
      geom_sf(data = sig_sf, fill = "black", alpha = 0.35, color = NA)
  }
...
# Each variable gets an annual map and a 2x2 seasonal panel.
# Figure numbering: 3.5a/3.5b (annual/seasonal) for the primary metric,
# then 3.6a/3.6b, 3.7a/3.7b, etc.
...
var_plot_config <- list(
  ...
  tke = list(
    fig_annual   = "Fig3.10a_Annual_Trend_Map_TKE.png",
    fig_seasonal = "Fig3.10b_Seasonal_Trend_Maps_TKE.png",
    unit_expr    = expression(hat(beta) ~ "[" * m^2 * s^{-2} * decade^{-1} * "]")
  )
)
...
for (vname in names(spatial_results)) {
  ...
  sig_ann   <- res$annual$significant
  slope_lim <- quantile(abs(slope_ann), 0.99, na.rm = TRUE)

  slope_stars <- make_stars_polar(slope_ann, "slope")
  sig_stars   <- make_stars_polar(
    matrix(as.integer(sig_ann), nrow(slope_ann), ncol(slope_ann)), "sig_mask"
  )
  ...
  p_ann <- make_panel(
    ...
    labs(subtitle = glue(
      "Pre-Whitened Mann\u2013Kendall | BH-FDR (\u03b1_FDR = {ALPHA_FDR}) ",
      "| Shading = FDR-significant"
    )) +
    theme(plot.subtitle = element_text(size = 8, colour = "grey40", hjust = 0.5))

  ggsave(file.path(FIG_PHASE3, cfg$fig_annual),
         p_ann, width = 8, height = 8, dpi = 300, bg = "white")
  progress_msg(sprintf("    Saved: %s", cfg$fig_annual))

  rm(slope_stars, sig_stars, p_ann)

  # ── Seasonal maps (2x2) ────────────────────────────────────────────────
  all_slopes_seas <- unlist(lapply(res$seasonal, function(x) x$slope))
  slope_lim_seas  <- quantile(abs(all_slopes_seas), 0.99, na.rm = TRUE)
  ...
  for (seas in SEASONS) {
    ...
    sg_st <- make_stars_polar(
      matrix(as.integer(sr$significant), n_lat, n_lon), "sig_mask"
    )
    ...
    fill_s <- scale_fill_distiller(
      ...
      palette   = "RdBu",
      direction = -1,
      limits    = c(-slope_lim_seas, slope_lim_seas),
      na.value  = "transparent",
      oob       = scales::squish
    )
    ...
    season_panels[[seas]] <- make_panel(
      ...
      fill_var    = "slope",
      fill_scale  = fill_s,
      panel_title = unname(SEASON_NAMES[seas]),
      sig_contour = sg_st
    )
  }
  ...
  fig_seas <- (season_panels[["DJF"]] + season_panels[["MAM"]] +
    ...
    plot_annotation(
      title = paste0("Seasonal Spatial Trends: Sen\u2019s Slope of ",
                     VAR_LABELS_PLAIN[[vname]], " (1958\u20132025)"),
      subtitle = glue(
        "Pre-Whitened Mann\u2013Kendall | BH-FDR (\u03b1_FDR = {ALPHA_FDR}) ",
        "| Shading = FDR-significant | Shared colour scale"
      ),
      theme = theme(
...
```{r}
load(file.path(OUTPUT_DATA_DIR, "Phase3_4_Spatial_Results.RData"))

# Check if fdr_summary exists (it's saved in the final save but not the intermediate ones)
if (exists("fdr_summary")) {
  print(fdr_summary, row.names = FALSE)
} else {
  # Rebuild it manually from spatial_results
  for (vname in names(spatial_results)) {
    res <- spatial_results[[vname]]
    
    # Annual
    n_sig_a <- sum(res$annual$significant, na.rm = TRUE)
    n_tot_a <- sum(!is.na(res$annual$p_local))
    cat(sprintf("%s | Annual | %d / %d sig (%.2f%%)\n",
                vname, n_sig_a, n_tot_a, 100 * n_sig_a / n_tot_a))
    
    # Seasonal
    for (seas in c("DJF", "MAM", "JJA", "SON")) {
      sr <- res$seasonal[[seas]]
      n_sig_s <- sum(sr$significant, na.rm = TRUE)
      n_tot_s <- sum(!is.na(sr$p_local))
      cat(sprintf("%s | %s    | %d / %d sig (%.2f%%)\n",
                  vname, seas, n_sig_s, n_tot_s, 100 * n_sig_s / n_tot_s))
    }
    cat("\n")
  }
}
...
```{r}
# Check what the seasonal aggregation actually produced
load(RESULTS_FILE)

# Pick a grid point with a strong visual slope in the MAM map (red region)
# and check the raw p-value BEFORE FDR correction
res <- spatial_results[["eke_synoptic_mean"]]

# Distribution of raw local p-values for MAM
p_mam <- res$seasonal[["MAM"]]$p_local
cat("MAM raw p-values summary:\n")
print(summary(as.vector(p_mam)))
cat(sprintf("\nMAM points with raw p < 0.05: %d / %d (%.1f%%)\n",
            sum(p_mam < 0.05, na.rm = TRUE),
            sum(!is.na(p_mam)),
            100 * sum(p_mam < 0.05, na.rm = TRUE) / sum(!is.na(p_mam))))

# Compare with DJF
p_djf <- res$seasonal[["DJF"]]$p_local
cat("\nDJF raw p-values summary:\n")
print(summary(as.vector(p_djf)))
cat(sprintf("\nDJF points with raw p < 0.05: %d / %d (%.1f%%)\n",
            sum(p_djf < 0.05, na.rm = TRUE),
            sum(!is.na(p_djf)),
            100 * sum(p_djf < 0.05, na.rm = TRUE) / sum(!is.na(p_djf))))

# Also check slopes
cat("\nMAM slopes summary:\n")
print(summary(as.vector(res$seasonal[["MAM"]]$slope)))
cat("\nDJF slopes summary:\n")
print(summary(as.vector(res$seasonal[["DJF"]]$slope)))
```
