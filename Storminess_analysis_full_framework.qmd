...
p1 <- ggplot(ts_long, aes(x = date, y = value, color = metric)) +
  ...
  labs(
    ...
    subtitle = "1958–2024 | Raw data (light) with LOESS smoother (bold)",
    x = NULL,
    y = NULL
  ) +
...
# ============================================================================
# PLOT 2: ACF PLOTS (ALL METRICS)
# ============================================================================

cat("\n--- Generating Plot 2: ACF Plots ---\n")

# Create multi-panel ACF plot
png(file.path(fig_dir, "Fig02_ACF_AllMetrics.png"),
    width = 14, height = 10, units = "in", res = 300)

# Set up layout with more space at top for main title
par(mfrow = c(2, 3), 
    mar = c(4, 4, 4, 1),      # bottom, left, top, right margins for each panel
    oma = c(0, 0, 3, 0))       # outer margins: extra space at top for main title

for (metric in METRICS) {
  if (metric %in% names(ts_objects)) {
    # Get rho1 value first
    rho1 <- acf(ts_objects[[metric]], lag.max = 1, plot = FALSE)$acf[2]
    
    # Create ACF plot
    acf(ts_objects[[metric]], lag.max = 36,
        main = paste0("ACF: ", metric),
        xlab = "Lag (months)",
        ylab = "Autocorrelation",
        col = metric_colors[metric],
        lwd = 2)
    
    # Add rho1 annotation using expression for proper subscript
    mtext(bquote(rho[1] == .(sprintf("%.3f", rho1))), 
          side = 3, line = -1.5, adj = 0.95, cex = 0.9)
  }
}

# Add overall title in outer margin
mtext("Autocorrelation Functions: Pan-Arctic Storminess Metrics (1958-2024)",
      outer = TRUE, line = 1, cex = 1.4, font = 2)

dev.off()
cat("  ✓ Saved: Fig02_ACF_AllMetrics.png\n")
...
p3_combined <- (p3a | p3b | p3c | p3d) +
  plot_annotation(
    ...
    subtitle = "Pan-Arctic (≥60°N) | 1958–2024 | Red dashed line = climatological 5% threshold",
    theme = theme(
...
p4_combined <- (p4a | p4b | p4c) +
  plot_annotation(
    ...
    subtitle = "Pan-Arctic (≥60°N) | 1958–2024 mean ± 1 SD",
    theme = theme(
      plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 11, hjust = 0.5)
    )
...
# Safeguard: Ensure lon and lat are simple vectors
# Use FULL lat vector, not lat_arctic
lon_vec <- as.vector(lon)
lat_vec <- as.vector(lat)  # Full latitude range

# Combine climatology matrices into a single list
# Transpose from [lat, lon] to [lon, lat] order for stars
data_list_plot <- list(
  eke_mean = t(climatology_maps_full$eke_mean),
  eke_p95 = t(climatology_maps_full$eke_p95),
  exceed_freq = t(climatology_maps_full$exceed_p95_freq)
)
...
p6_combined <- p6a + p6b + p6c +
  plot_layout(ncol = 3) +
  plot_annotation(
    title = "Spatial Climatology of Storminess Metrics (1958-2024)",
    subtitle = "Full domain visualization (≥56°N) | Lambert Azimuthal Equal-Area Projection",
    theme = theme(
...
p7 <- ggplot(dist_data, aes(x = value, fill = metric)) +
  ...
  labs(
    ...
    subtitle = "Pan-Arctic (≥60°N) | 1958–2024 | n = 804 months",
    x = "Value",
    y = "Density"
  ) +
  theme_bw(base_size = 11) +
...
p9_combined <- (p9a / p9b / p9c) +
  plot_annotation(
    ...
    subtitle = "Pan-Arctic (≥60°N) | Monthly anomalies from 1958–2024 climatology",
    theme = theme(
      plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 11, hjust = 0.5)
    )
