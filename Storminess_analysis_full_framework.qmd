...
for (vname in names(spatial_results)) {
  ...
  # Pool all slopes for this variable across every period so that the
  # annual map and the seasonal 2x2 panel share an identical colour range.
  # The 0.99 quantile of the combined set is used so that a handful of
  # extreme grid-point values do not collapse the palette for everything
  # else (oob = scales::squish handles the residual outliers).
  all_slopes_var <- c(res$annual$slope,
                      unlist(lapply(res$seasonal, function(x) x$slope)))
  slope_lim_var  <- quantile(abs(all_slopes_var), 0.99, na.rm = TRUE)
