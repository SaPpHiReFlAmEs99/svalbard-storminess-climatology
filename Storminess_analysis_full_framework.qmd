...
**Pre-Whitened Mann-Kendall (PW-MK) Test:**
- Accounts for temporal autocorrelation via pre-whitening: $Y'_t = Y_t - \rho_1 Y_{t-1}$
- Where $\rho_1$ is the lag-1 autocorrelation coefficient
- Non-parametric test for monotonic trends (no distributional assumptions)
- Null hypothesis: No trend exists

### 2. Trend Magnitude
**Theil-Sen Slope Estimator:**
- Robust to outliers (uses median of all pairwise slopes)
- Units: m²/s² per decade (MKE, EKE) or dimensionless per decade (ratio)
- Reported as: Change per decade over the 67-year period

### 3. Uncertainty Quantification
**Moving Blocks Bootstrap** ($R = 5000$ replicates):
- **Block length selection** (critical for preserving autocorrelation structure):
  - **l = 3 years** for all annual and seasonal analyses
  - Rationale: Annual data has no seasonal cycle; block length must preserve inter-annual persistence typical of Arctic climate (NAO, AMO influence ~2-3 years)
- Percentile method: 95% confidence interval from bootstrap distribution
- Trend considered significant if: (1) $p < 0.05$ AND (2) 95% CI excludes zero

### Verification of Aggregation Success
Lag-1 autocorrelations ($\rho_1$) confirm proper annual aggregation:
- **Expected for annual data:** $0.0 \le |\rho_1| \le 0.3$ (weak year-to-year correlation)
- **Observed:**
  - MKE: $\rho_1 \in [-0.025, 0.204]$ ✓
  - EKE: $\rho_1 \in [0.059, 0.122]$ ✓  
  - Ratio: $\rho_1 \in [-0.059, 0.098]$ ✓
- **Contrast with monthly data:** Would show $\rho_1 > 0.5$ due to seasonal cycle
...
The Arctic atmosphere exhibited **comprehensive dynamical stability** across the 67-year period (1958-2024), with no detectable trends in:
- Distribution moments (mean, variance)
- Extreme values (P95, P99)
- Energy budget components (MKE, EKE, ratio)
- Temporal scales (annual, all seasons)
- Spatial domains (Pan-Arctic, regional)

This stability persisted despite:
- Dramatic Arctic sea ice loss (>50% summer extent)
- Significant Arctic amplification of warming (2-3× global rate)
- Major changes in Arctic Ocean and sea ice conditions
