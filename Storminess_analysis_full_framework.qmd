...
## 2.4 Regional Sub-Analysis: Svalbard Trend
...
```{r eval=FALSE}
# ============================================================================
# PHASE 4.1 - SUB-ANALYSIS: SVALBARD REGIONAL TREND
# ============================================================================
...
lon_svalbard <- which(lon >= -20 & lon <= 35)
...
```{r eval=FALSE}
# ============================================================================
# PHASE 4.2 - SUB-ANALYSIS: "SVALBARD SECTOR" (Land + Ocean)
# Using Pre-Whitened Mann-Kendall for Significance
# Includes Save/Load logic for bootstrap results
# ============================================================================

# --- 0. Load Required Libraries ---
library(trend)      # For sens.slope() and mk.test()
library(boot)       # For tsboot()

cat("\n", rep("=", 70), "\n")
cat("ANALYZING SVALBARD SECTOR (Land + Ocean) TIME SERIES\n")
cat(rep("=", 70), "\n\n")

# --- 1. Define the Svalbard Sector Bounding Box ---
...
cat("✓ Area-weighted time series for Svalbard Sector created.\n")
...
rho1_sector <- acf(storm_ts_obj_sector, plot = FALSE)$acf[2]
cat(sprintf("\nLag-1 Autocorrelation (ρ₁): %.3f\n", rho1_sector))

# --- 4. Pre-Whitened Mann-Kendall test for significance (Yue et al. 2002) ---
cat("\n--- Pre-Whitened Mann-Kendall Test (Svalbard Sector) ---\n")
...
cat("\n--- Sen's Slope Estimator (Svalbard Sector) ---\n")
sens_sector <- sens.slope(storm_ts_obj_sector)
slope_per_year_sector <- sens_sector$estimates # units/year
slope_decade_sector <- slope_per_year_sector * 10
cat(sprintf("  Slope (per decade): %.4f m⁴/s⁴\n", slope_decade_sector))

# --- 6. Moving Blocks Bootstrap (with Save/Load) ---
cat("\n--- Moving Blocks Bootstrap (Svalbard Sector) ---\n")
...
block_length_sector <- 12 
cat(sprintf("  Using fixed, justified block length (l): %d months\n", block_length_sector))

# *** NEW: Define a path to save/load the Svalbard bootstrap results ***
processed_dir <- "E:/ERA5_data/processed"
...
if (!inherits(boot_ci_sector, "try-error") && !is.null(boot_ci_sector$percent) && all(!is.na(boot_ci_sector$percent[4:5]))) {
  ...
  if (lower_ci_decade < 0 && upper_ci_decade > 0) {
    cat("  -> Bootstrap CI includes zero.\n")
  } else {
    cat("  -> Bootstrap CI does NOT include zero.\n")
  }
} else {
  cat("Bootstrap 95% CI: Calculation failed (check bootstrap replicates for NAs).\n")
  print(summary(boot_result_sector$t))
}

# Final conclusion based on the PW-MK test
if (is_significant_sector) {
  direction <- ifelse(sens_sector$estimates > 0, "INCREASING", "DECREASING")
  cat(sprintf("\nCONCLUSION (PW-MK Test): A STATISTICALLY SIGNIFICANT %s TREND DETECTED.\n", direction))
} else {
  cat("\nCONCLUSION (PW-MK Test): No statistically significant trend detected for the Svalbard Sector.\n")
}
```
