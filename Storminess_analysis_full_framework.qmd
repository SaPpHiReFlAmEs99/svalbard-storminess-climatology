...
This diagnostic is the single most important prerequisite for the entire statistical framework. Standard statistical tests, including Ordinary Least Squares (OLS) regression, operate on the strict assumption that the data points (or model residuals) are independent. Climate time series violate this assumption due to strong persistence, or "memory," arising from the inertia of the climate system.

### 1.4.1 The Autocorrelation Function (ACF)

To quantify the degree of persistence in the area-weighted Pan-Arctic storminess metrics, the Autocorrelation Function (ACF) was computed for lags up to 24 months. The ACF measures the correlation between observations separated by $k$ time steps:

$$\rho_k = \frac{\text{Cov}(X_t, X_{t-k})}{\text{Var}(X_t)}$$

### 1.4.2 Effective Sample Size
...
|-----------------|----------------------|-----------|---------------|
| eke_mean        | 0.648                | 592       | 27.4%         |
| eke_p90         | 0.649                | 599       | 26.6%         |
| eke_p95         | 0.658                | 595       | 27.1%         |
| eke_p99         | 0.664                | 600       | 26.5%         |
| exceed_p95_freq | 0.583                | 571       | 30.1%         |

: Autocorrelation diagnostics for Pan-Arctic storminess metrics {#tbl-acf}

All lag-1 values substantially exceed the threshold of $\rho_1 > 0.1$ that indicates violation of the independence assumption for monthly atmospheric data (Santer et al. 2000). The consistency of autocorrelation structure across metrics (all $\rho_1 \approx 0.6$) confirms that this persistence reflects genuine atmospheric dynamics rather than metric-specific artifacts.

The approximately 27% reduction in degrees of freedom has critical implications for hypothesis testing: standard errors will be underestimated and p-values will be artificially small if autocorrelation is ignored. Consequently, all subsequent trend analyses employ autocorrelation-corrected methods, specifically the pre-whitened Mann-Kendall test and GLS regression with AR(1) error structure.

```{r}
# ============================================================================
# Section 1.4: Assessment of Temporal Autocorrelation
# ============================================================================
