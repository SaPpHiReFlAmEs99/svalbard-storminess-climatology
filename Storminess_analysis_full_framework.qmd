...
for (vname in names(spatial_results)) {
  ...
  # ── Unified colour-scale limit (annual + all four seasons) ─────────────
  # Take the MAXIMUM of the five per-period 0.99 quantiles rather than the
  # 0.99 quantile of the pooled distribution.  Pooling mixes annual slopes
  # (12-month averages, small variance) with seasonal slopes (3-month
  # averages, much larger variance): a season like DJF can have 5-10% of
  # its own values above the pooled 0.99 quantile, producing large solid
  # colour patches.  Using the per-period max ensures that within any
  # individual panel at most ~1% of its own grid points are squished.
  slope_lim_var <- max(
    quantile(abs(res$annual$slope), 0.99, na.rm = TRUE),
    sapply(res$seasonal, function(x) quantile(abs(x$slope), 0.99, na.rm = TRUE))
  )
  breaks_var     <- pretty_breaks_sym(slope_lim_var, n = 7L, max_dec = cfg$max_dec)
  ...
  fill_scale <- scale_fill_distiller(
    ...
    palette   = "RdBu",
    direction = -1,
    limits    = range(breaks_var),   # span the 7 breaks exactly; no break is dropped
    breaks    = breaks_var,
  ...
  for (seas in SEASONS) {
    ...
    fill_s <- scale_fill_distiller(
      ...
      palette   = "RdBu",
      direction = -1,
      limits    = range(breaks_var),   # same limits as annual; no break is dropped
      breaks    = breaks_var,
      labels    = scales::label_number(accuracy = 5 * 10^(-cfg$max_dec)),
      na.value  = "transparent",
      oob       = scales::squish
