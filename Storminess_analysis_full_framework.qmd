...
shared_max <- max(
  ...
  max(clim_polar_masked[["eke_p90"]],           na.rm = TRUE),
  max(clim_polar_masked[["eke_p95"]],           na.rm = TRUE),
  max(clim_polar_masked[["eke_p99"]],           na.rm = TRUE)
)
...
# FIGURE 6a: MEAN EKE + P90 EKE + P95 EKE + EXCEEDANCE FREQUENCY  (2×2 layout)
# ==========================================================================
cat("\n--- Generating Figure 6a: Spatial Climatology 2x2 (Mean / P90 / P95 / Exceedance) ---\n")
...
# Shared scale spec used for all three EKE intensity panels
eke_scale <- scale_fill_viridis(
...
p6a_p90 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('bold("(b) "~', VAR_LABELS_SHORT[["eke_p90"]], ')'))[[1]]) +
  panel_theme
...
p6a_p95 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('bold("(c) "~', VAR_LABELS_SHORT[["eke_p95"]], ')'))[[1]]) +
  panel_theme
...
p6a_p99 <- ggplot() +
  ...
  coord_sf(xlim = c(-plot_radius, plot_radius),
           ylim = c(-plot_radius, plot_radius), expand = FALSE) +
  labs(title = parse(text = paste0('bold("(d) "~', VAR_LABELS_SHORT[["eke_p99"]], ')'))[[1]]) +
  panel_theme

# --- Assemble 2×2 ---
# guides = "collect" merges the three identical EKE colourbars into one;
# the exceedance panel keeps its own separate legend.
fig6a <- (p6a_mean | p6a_p90) / (p6a_p95 | p6a_p99) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title    = "Spatial Climatology of EKE (1958–2025)",
    subtitle = "Pan-Arctic (≥55°N) | Lambert Azimuthal Equal-Area Projection | Shared colour scale for all panels",
    theme = theme(
      plot.title    = element_text(size = TITLE_SIZE, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = SUBTITLE_SIZE, hjust = 0.5)
    )
...
ggsave(file.path(fig_dir, "Fig06a_EKE_Intensity_Climatology.png"),
       fig6a, width = 14, height = 14, dpi = 300, bg = "white")
...
ggsave(file.path(fig_dir, "Fig06a2_P90EKE_Polar.png"),
       p6a_p90, width = 8, height = 8, dpi = 300, bg = "white")
ggsave(file.path(fig_dir, "Fig06a3_P95EKE_Polar.png"),
       p6a_p95, width = 8, height = 8, dpi = 300, bg = "white")
ggsave(file.path(fig_dir, "Fig06a4_P99EKE_Polar.png"),
       p6a_p99, width = 8, height = 8, dpi = 300, bg = "white")
